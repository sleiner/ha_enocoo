...
class StatisticsInserter:
    ...
    async def _insert_individual_photovoltaic_statistics(
        ...
        statistic_id = self._statistic_id(metric.id_suffix, area=area).replace(
            ":", ":zz99"
        )  # TODO
        (
            expecting_newer_data,
...
class IndividualCalculatedMetric(ABC):
    ...
    async def get_daily_datapoints(self, area: Area, date: dt.date) -> list[Datapoint]:
        ...
        datapoints = []
        for d in (consumption_by_hour, pv_stats_by_hour):
            if not d.keys():
                print("oy")
        last_hour = min(
            ...
            try:
                ...
                aggregate_pv = PhotovoltaicSummary(
                    ...
                    own_consumption=Quantity(
                        ...
                        unit=all_the_same(
                            pv.own_consumption.unit for pv in pv_stats if pv is not None
                        ),
                    ),
...
class IndividualSuppplyFromPhotovoltaic(IndividualCalculatedMetric):
    @override
    @property
    def id_suffix(self) -> str:
    ...
    def calculate_datapoint(
        ...
        return Datapoint(
            start=start,
            period=period,
            value=electricity_consumption.value * pv_supply_ratio,
            unit=electricity_consumption.unit,
        )


class IndividualSuppplyFromGrid(IndividualCalculatedMetric):
    @override
    @property
    def id_suffix(self) -> str:
    ...
    def calculate_datapoint(
        ...
        return Datapoint(
            start=start,
            period=period,
            value=electricity_consumption.value * (1 - pv_supply_ratio),
            unit=electricity_consumption.unit,
        )
